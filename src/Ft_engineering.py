# ==============================================
# ft_engineering.py - Ingeniería de características (V5.0 - FINAL LOGS)
# ==============================================
...
def load_data():
    ...
    # Como vimos en el EDA, hay basura numérica.
    if 'tendencia_ingresos' in df.columns:
        validos = ['Creciente', 'Decreciente', 'Estable']
        # Si no es válido, lo convertimos a NaN (el Pipeline luego lo llenará como "Desconocido")
...
def create_safe_features(df):
    ...
    if 'edad_cliente' in df.columns:
        ...
        # Fix edad 123: Tratamos > 90 como 90
    ...
    if 'cuota_pactada' in df.columns and 'total_otros_prestamos' in df.columns:
        ...
        if 'salario_cliente' in df.columns:
            ...
            # Clipamos valores extremos absurdos
...
def make_train_test_data(target_col='Pago_atiempo', split_ratio=0.8):
    ...
    # A) Columnas Sesgadas (Salarios, Préstamos) -> LOGARITMO + ROBUST
    # Estas son las que tienen los outliers de 22 mil millones
    skewed_cols = [
    ...
    skewed_transformer = Pipeline(steps=[
        ...
        ('log', FunctionTransformer(np.log1p, validate=False)), # Log(x+1) aplasta los billones
        ('scaler', RobustScaler()) # Robusto ignora lo que quede de outlier
    ])
    
    # B) Resto de numéricas -> SOLO ROBUST
    ...
    # C) Categóricas -> IMPUTAR "DESCONOCIDO" + ONEHOT
    # Aquí recuperamos los nulos de tendencia_ingresos
...
if __name__ == "__main__":
    try:
        ...
        # Verificar que la tendencia de ingresos generó la columna "Desconocido"
